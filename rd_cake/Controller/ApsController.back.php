...
class ApsController extends AppController {
               ...
               //// print_r($ap_profile);
                ...
                if($include_new_server){
                    $this->set(array(
                        ...
                        'success' => false,
                        '_serialize' => array('new_server','AP attaced to server success')
                    ));
                }else{
                     $this->set(array(
    ...
    private function _build_network($ap_profile){
		...
		$inner_lan_data			= array(); //insert inner lan network
		$wan_data			= array(); //insert lan network
        ...
        foreach($ap_profile['ApProfileExit'] as $ap_profile_e){
            ...
            if(count($ap_profile_e['ApProfileExitApProfileEntry']) > 0){
                ...
                foreach($ap_profile_e['ApProfileExitApProfileEntry'] as $entry){    
					...
					elseif($type == 'lan'){
						array_push($entry_point_data,array('network' => 'lan','entry_id' => $entry['ap_profile_entry_id'])); //lan inner
					}
					elseif($type == 'wan'){
						array_push($entry_point_data,array('network' => 'wan','entry_id' => $entry['ap_profile_entry_id'])); //wan ethernet
                    }else{
                        array_push($entry_point_data,array('network' => $if_name,'entry_id' => $entry['ap_profile_entry_id'])); //zero - Mesh Network
                    }        
                }
            }
                    ...
                    if(
                        ...
                        ($ap_profile_e['frequency_band'] == $band)){
                            ...
                            $base_array = array(
										...
										"rsn_preauth"   => $ap_profile_e['rsn_preauth'],
										"ieee80211r"   => $ap_profile_e['ieee80211r'],
										"ft_over_ds"   => $ap_profile_e['ft_over_ds'],
										"ft_psk_generate_local"   => $ap_profile_e['ft_psk_generate_local'],
										"nasid"   => $ap_profile_e['nasid']
                                   );
